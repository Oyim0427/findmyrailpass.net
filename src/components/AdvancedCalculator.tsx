...
import { MAJOR_REGIONS, type JRPass } from '@/types/pass';
...
const SEARCH_HISTORY_KEY = 'findmyrailpass-calculator-search-history';
const SEARCH_HISTORY_LIMIT = 8;
const regions = ['全国', ...MAJOR_REGIONS];

function readSearchHistory(): string[] {
  if (typeof window === 'undefined') return [];
  try {
    const stored: unknown = JSON.parse(window.localStorage.getItem(SEARCH_HISTORY_KEY) || '[]');
    if (!Array.isArray(stored)) return [];
    const entries = stored
      .filter((value): value is string => typeof value === 'string')
      .map(value => value.trim().replace(/\s+/g, ' ').slice(0, 100))
      .filter(Boolean);
    return entries.filter((value, index) => entries.findIndex(item => item.toLowerCase() === value.toLowerCase()) === index).slice(0, SEARCH_HISTORY_LIMIT);
  } catch {
    return [];
  }
}
...
const copy = {
  zh: {
    ...
    history: '最近搜索', clearHistory: '清除履历',
    ...
    resultDesc: '优先显示同时匹配出发地和旅行地的券，再显示范围更广的券，最后显示仅匹配一端的候选。有效天数用于同级排序；地方券的大区归类不代表线路已核实。',
    ...
    reasons: { regionMatch: '官方范围包含旅行地区', nationalMatch: '全国型票券，请核对具体线路', originMatch: '官方范围包含出发地区', bothFocused: '大区与出发地、旅行地均匹配', bothBroad: '两端均匹配，范围还包括其他大区', destinationOnly: '仅匹配旅行地，抵达前可能需另购票', originOnly: '仅匹配出发地，后续可能需另购票', daysFit: '有效天数与行程相同', daysShort: '有效期短于行程，需集中使用', daysLong: '有效期长于行程', directoryRegion: '按目录大区列为候选，非已核实线路', directoryReview: '价格、日期及利用条件待官网核验' },
  },
  en: {
    ...
    history: 'Recent searches', clearHistory: 'Clear history',
    ...
    resultDesc: 'Passes matching both endpoints come first, followed by broader passes and then one-sided candidates. Validity breaks ties. Directory regions are not verified route coverage.',
    ...
    reasons: { regionMatch: 'Official area includes the travel region', nationalMatch: 'Nationwide pass; check individual routes', originMatch: 'Official area includes the origin', bothFocused: 'Major regions match origin and destination', bothBroad: 'Both match; the pass includes other regions too', destinationOnly: 'Destination only; travel there may need another ticket', originOnly: 'Origin only; onward travel may need another ticket', daysFit: 'Validity matches trip length', daysShort: 'Shorter validity; concentrate travel days', daysLong: 'Validity is longer than the trip', directoryRegion: 'Directory-region candidate; route coverage unverified', directoryReview: 'Confirm fares, dates and eligibility with the operator' },
  },
  ja: {
    ...
    history: '最近の検索', clearHistory: '履歴を削除',
    ...
    resultDesc: '出発地と旅行地の両方に一致する券、より広域の券、片方のみ一致する候補の順に表示します。同順位では有効日数を考慮します。地方きっぷの掲載地域は利用可能路線の確認ではありません。',
    ...
    reasons: { regionMatch: '公式の利用範囲に旅行エリアを含む', nationalMatch: '全国型パス・個別の路線を要確認', originMatch: '公式の利用範囲に出発エリアを含む', bothFocused: '出発地・旅行地の両地域に一致', bothBroad: '両地域に一致し、他の地域も含む', destinationOnly: '旅行地のみ一致・到着前は別のきっぷが必要な場合あり', originOnly: '出発地のみ一致・先の区間は別のきっぷが必要な場合あり', daysFit: '有効日数と旅行日数が一致', daysShort: '旅行より有効期間が短いため集中利用が必要', daysLong: '旅行より有効期間が長い', directoryRegion: '目録の地域による候補・利用路線は未確認', directoryReview: '価格・日付・利用条件を公式サイトで要確認' },
  },
};
...
export default function AdvancedCalculator({ passes, lang = 'zh' }: AdvancedCalculatorProps) {
  ...
  const rememberSearch = (value: string) => {
    ...
    if (!keyword) return;
    const nextHistory = [keyword, ...searchHistory.filter(item => item.toLowerCase() !== keyword.toLowerCase())].slice(0, SEARCH_HISTORY_LIMIT);
  ...
  const comparePasses = (keyword = query) => {
    const budget = Number(ticketBudget) > 0 ? Number(ticketBudget) : undefined;
    setResults(searchCalculatorCatalog(catalog, { origin, destination, query: keyword, category, tripDays, adults: travellers, children: childrenCount, budget }));
  ...
  const clearSearchHistory = () => {
    ...
    try {
      ...
      // Clearing the visible history still works when storage is unavailable.
    }
  };
      ...
      <form onSubmit={(e) => { e.preventDefault(); comparePasses(); }} onChange={() => { setResults(null); setVisibleCount(RESULT_PAGE_SIZE); }} className="grid gap-5 p-6 sm:grid-cols-2 sm:p-9 lg:grid-cols-3">
        ...
        >
          ...
          <input
            ...
            aria-expanded={searchOpen && (query.trim().length > 0 || searchHistory.length > 0)}
            aria-controls="pass-keyword-suggestions"
            autoComplete="off"
            className="field-select"
          />
          {searchOpen && (query.trim().length > 0 || searchHistory.length > 0) && (
            <div id="pass-keyword-suggestions" aria-label={query.trim() ? t.suggestions : t.history} className="absolute inset-x-0 top-full z-30 mt-2 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_18px_55px_rgba(15,23,42,0.18)]">
              {!query.trim() ? (
                <>
                  <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50 px-4 py-2">
                    <p className="text-xs font-semibold text-slate-600">{t.history}</p>
                    <button type="button" onClick={clearSearchHistory} className="text-xs font-semibold text-primary-dark hover:underline">{t.clearHistory}</button>
                  </div>
                  <div className="max-h-[22rem] overflow-y-auto">
                    {searchHistory.map(keyword => (
                      <button
                        key={keyword}
                        type="button"
                        onClick={() => { setQuery(keyword); comparePasses(keyword); }}
                        className="flex w-full items-center gap-3 border-b border-slate-100 px-4 py-3 text-left text-sm text-slate-800 last:border-0 hover:bg-primary/5 focus:bg-primary/5 focus:outline-none"
                      >
                ...
                <>
                  ...
                  <div className="max-h-[22rem] overflow-y-auto">
                    {keywordSuggestions.map(pass => {
                      ...
                      const region = (directory ? pass.majorRegions.length ? pass.majorRegions : [pass.region] : pass.coverage.regions)
                        .map(value => getRegionLabel(value, locale)).join(' / ');
                      return (
                        <Link
                          key={directory ? `directory:${pass.id}` : `verified:${pass.id}`}
                          href={getCatalogDetailHref(pass, locale)}
                          onClick={() => { rememberSearch(query); setSearchOpen(false); }}
          ...
          return <article key={result.key} className="min-w-0 overflow-hidden rounded-2xl border border-slate-200 bg-white p-5 sm:p-6">
            ...
            <div className="flex flex-col justify-between gap-5 md:flex-row">
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-2 text-sm text-slate-500">{pass.company} · {(directory ? pass.majorRegions.length ? pass.majorRegions : [pass.region] : pass.coverage.regions).map(value => getRegionLabel(value, locale)).join(' / ')} · {pass.category === 'local' ? t.local : getDirectoryPassCategoryLabel(pass.category, locale)}</p>
