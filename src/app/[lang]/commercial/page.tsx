...
export default async function CommercialPage({ params }: { params: Promise<{ lang: string }> }) {
  ...
  return <main className="min-h-screen px-4 py-16"><article className="mx-auto max-w-3xl rounded-3xl border border-slate-200 bg-white p-8 shadow-sm sm:p-12"><Link href={`/${lang}`} className="inline-flex items-center gap-2 text-sm font-bold text-primary"><ArrowLeft className="h-4 w-4" />返回首页</Link><h1 className="mt-8 text-4xl font-bold text-slate-950">特定商取引法に基づく表記 / 商业交易披露</h1>
...
function Row({ k, v }: { k: string; v: string }) { return <div className="grid border-b border-slate-200 last:border-0 sm:grid-cols-[11rem_1fr]"><div className="bg-primary/10 p-4 font-bold text-slate-700">{k}</div><div className="p-4 leading-6 text-slate-600">{v}</div></div>; }
